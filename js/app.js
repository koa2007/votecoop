...
const app = {
  ...
  async initWithSupabase() {
    ...
    const _pwResetToken = new URLSearchParams(window.location.search).get(
      "pwreset",
    );
    if (_pwResetToken) {
      ...
      try {
        supabaseService.pb.authStore.clear();
      } catch (e) {}
  ...
  async handleAuthSession(session) {
    ...
    if (error) {
      ...
      if (!restoredUser) {
        ...
        const t = this.translations[this.currentLanguage] || {};
        this.toastError(
          t.load_failed || "Не вдалося завантажити. Спробуйте ще раз.",
        );
        this.showAuthScreen();
        return;
      }
    ...
    if (restoredUser || (profile && profile.profile_completed)) {
      ...
      try {
        localStorage.setItem("vc_user", JSON.stringify(this.state.user));
      } catch (e) {
        /* ignore */
      }
      ...
      try {
        ...
        await Promise.all([this.loadMyVotings(), this.loadMyNotifications()]);
      } catch (loadErr) {
  ...
  _showScreenInternal(screenId) {
    ...
    if (topLevelScreens.includes(screenId) || screenId === "main-screens") {
      ...
      if (screenId === "main-screens") {
        ...
        document
          .querySelectorAll("#main-screens > .screen")
  ...
  showResetPasswordScreen() {
    ...
    ["reset-password-old", "reset-password-1", "reset-password-2"].forEach(
      (id) => {
        const el = document.getElementById(id);
        if (el) el.value = "";
      },
  ...
  async submitNewPassword() {
    ...
    if (p1 !== p2) {
      this._showInlineError(
        "reset",
        t.reset_mismatch || "Паролі не співпадають",
      );
      return;
    }
    ...
    if (!this._pwResetToken) {
      ...
      if (!oldPw) {
        this._showInlineError(
          "reset",
          t.reset_old_required || "Введіть поточний пароль",
        );
        return;
      }
    }
    ...
    if (error) {
      ...
      this._showInlineError(
        ...
        fields.oldPassword
          ...
          : fields.password
            ? t.auth_error_password_short
            : this.humanError(error),
      );
      return;
    }
  ...
  async registerWithEmail() {
    ...
    const emailEl = document.getElementById(
      onRegScreen ? "register-email" : "auth-email",
    );
    const passEl = document.getElementById(
      onRegScreen ? "register-password" : "auth-password",
    );
    const btnId = onRegScreen ? "register-submit-btn" : "auth-register-btn";
    const showErr = (m) =>
      onRegScreen ? this.showRegisterError(m) : this.showAuthError(m);
    const showOk = (m) =>
      onRegScreen ? this.showRegisterSuccess(m) : this.showAuthSuccess(m);
    const hideMsg = () =>
      onRegScreen ? this.hideRegisterMessages() : this.hideAuthMessages();
    ...
    if (error) {
      ...
      const fields = (error.data && error.data.data) || {};
      if (
        fields.email?.code === "validation_not_unique" ||
  ...
  async saveProfile() {
    ...
    const needsTerms =
      (this.state.user?.termsVersion || 0) < this.TERMS_VERSION;
  ...
  async acceptTerms() {
    ...
    if (error) {
      this.toastError(
        t.terms_accept_error ||
          "Не вдалося зберегти згоду. Перевірте інтернет і спробуйте ще раз.",
      );
      return;
    }
  ...
  async loadMyGroups() {
      ...
      if (error && !this.state.groups.length) {
        this.toastError(
          (this.translations[this.currentLanguage] || {}).load_failed ||
            "Не вдалося завантажити. Потягніть вниз, щоб оновити.",
        );
      }
      this.renderGroups();
      return;
    ...
    try {
      ...
      /* non-fatal — defaults to voter */
    }

    // Cache for next load
    try {
      localStorage.setItem("vc_groups", JSON.stringify(this.state.groups));
    } catch (e) {
      /* storage full */
    }
  ...
  async loadMyVotings() {
    ...
    if (votingIds.length > 0) {
      ...
      votedSet = new Set(
        (myVotes || []).map((mv) => mv.voting_id).filter((id) => ids.has(id)),
      );
    }

    this.state.votings = votings.map((v) => {
      const r = resultsMap[v.id] || {
        yes_votes: 0,
        no_votes: 0,
        abstain_votes: 0,
        total_votes: 0,
      ...
      return {
        ...
        targetMemberName: v.target
          ? `${v.target.first_name} ${v.target.last_name}`.trim()
          : null,
        removalReason: v.removal_reason,
        initiatorId: v.created_by,
        initiatorName: v.creator
          ? `${v.creator.first_name} ${v.creator.last_name}`.trim()
          : "",
        freezeMembers: [],
        objections: [],
        comments: [],
  ...
  subscribeToRealtime() {
    ...
    supabaseService.realtimeSubscribe("notifications", (e) => {
      ...
      if (n.type === "new_voting" || n.type === "voting_completed") {
        ...
        n.type === "join_request" ||
        n.type === "role_change_request"
      ) {
        if (
          this.state.currentScreen === "group-detail-screen" &&
  ...
  unsubscribeFromRealtime() {
    ...
    try {
      ["notifications", "votes", "votings", "join_requests"].forEach((c) =>
        supabaseService.realtimeUnsubscribe(c),
      );
    } catch (e) {
      /* ignore */
    }
  ...
  renderNotifications() {
    ...
    list.innerHTML = this.state.notifications
      .map((notif) => {
        ...
        const isJoinRequest =
          (notif.type === "join_request" || notif.type === "member") &&
          meta.request_id &&
          meta.group_id;
  ...
  async approveFromNotification(
    ...
    const { error } = await supabaseService.approveJoinRequestV2(
      requestId,
      forceObserver,
    );
    if (error) {
      ...
      if (
        (msg + JSON.stringify(error.data || "")).includes("request_not_found")
      ) {
  ...
  async rejectFromNotification(notifId, groupId, requestId) {
    ...
    if (error) {
      if (
        ((error.message || "") + JSON.stringify(error.data || "")).includes(
          "request_not_found",
        )
      ) {
        await this._clearRequestButtons(requestId);
  ...
  async _clearRequestButtons(requestId) {
    ...
    await Promise.all(
      affected.map((n) =>
        supabaseService.updateNotificationMetadata(n.id, n.metadata),
      ),
    );
  },
  ...
  async archiveAllNotifications() {
    ...
    const ok = await this.confirm({
      ...
      message:
        ...
        `Перенести ${count} сповіщень в архів? Вони залишаться в історії, але список очиститься.`,
    ...
    if (error) {
      ...
      if (/archived_at/i.test(error.message || "")) {
        this.toastError(
          t.archive_needs_migration ||
            "Спочатку накатіть phase12-notif-archive.sql",
        );
        return;
      }
      this.toastError(this.humanError(error));
  ...
  async _fetchArchivePage(append) {
    ...
    const { data, error } = await supabaseService.searchNotifications(
      arch.query,
      true,
      arch.pageSize,
      arch.offset,
  ...
  async unarchiveNotification(id) {
    ...
    const ok = await this.confirm({
      ...
      message:
        t.unarchive_confirm || "Повернути сповіщення до основного списку?",
  ...
  humanError(error) {
    ...
    try {
      raw += " " + JSON.stringify(error?.data || error?.response || "");
    } catch (e) {
      /* ignore */
    }
    const map = {
    ...
    for (const k in map) {
      if (raw.includes(k) && map[k]) return map[k];
    }
    return (
      t.error_generic ||
  ...
  initPullToRefresh() {
    ...
    document
      .querySelectorAll(".screen-content[data-ptr]")
      .forEach((container) => {
        const route = container.getAttribute("data-ptr");
        const indicator = document.createElement("div");
        indicator.className = "ptr-indicator";
        indicator.innerHTML =
          '<i class="ph ph-arrow-down"></i><span class="ptr-text"></span>';
  ...
  async submitFeedback() {
    ...
    if (text.length < 5) {
      this.toastError(
        t.feedback_too_short || "Напишіть більше деталей (мін. 5 символів)",
      );
      return;
    }
    ...
    this.toastSuccess(
      t.feedback_thanks ||
        "Дякуємо! Ми отримали ваш відгук — найближчим часом розглянемо.",
    );
  },
  ...
  async showAdminPanel() {
    if (!this.isAdmin()) {
      this.toastError(
        (this.translations[this.currentLanguage] || {}).err_not_admin ||
  ...
  _setupAdminTabs() {
    ...
    document.querySelectorAll(".segment[data-admin-tab]").forEach((seg) => {
      seg.addEventListener("click", () => {
        ...
        document
          .querySelectorAll(".segment[data-admin-tab]")
          .forEach((s) => s.classList.remove("active"));
  ...
  async refreshAdminPanel() {
    ...
    const t = this.translations[this.currentLanguage] || {};
    if (grid)
      grid.innerHTML = `<div class="empty-state">${this.escapeHTML(t.loading || "Loading…")}</div>`;
      ...
      if (grid)
        grid.innerHTML = [
          tile(
            "Всього користувачів",
            s.users_total,
            `${s.users_completed || 0} завершили профіль`,
          ),
          tile(
            "Нові за тиждень",
            s.users_last_7d,
            `${s.users_last_24h || 0} за добу`,
          ),
          ...
          tile(
            "Голосувань",
            s.votings_total,
            `${s.votings_active || 0} активних`,
          ),
          tile(
            "Завершені",
            s.votings_completed,
            `${s.votings_accepted || 0} прийнято / ${s.votings_rejected || 0} відхилено`,
          ),
    ...
    if (ut)
      ut.innerHTML =
        usersList || '<div class="empty-state">Немає користувачів</div>';
    ...
    if (gt)
      gt.innerHTML = groupsList || '<div class="empty-state">Немає груп</div>';
    ...
    if (ft)
      ft.innerHTML = fbList || '<div class="empty-state">Немає відгуків</div>';
  },
  ...
  async sendFeedbackReply(id) {
    ...
    if (reply.length < 2) {
      this.toastError(
        (this.translations[this.currentLanguage] || {}).reply_too_short ||
          "Напишіть відповідь",
      );
      return;
    }
  ...
  onInstalled() {
    ...
    const t = this.translations[this.currentLanguage] || {};
    this.toastSuccess(
      t.install_thanks || "Дякуємо! Тепер Spilka на головному екрані.",
    );
  },
  ...
  async installApp() {
    ...
    if (isIOS) {
      this.toastInfo(
        t.install_ios_hint ||
          "На iOS: натисніть «Поділитися» → «На екран Домівка»",
        6000,
      );
      return;
    }
    ...
    if (!evt) {
      ...
      this.toastInfo(
        t.install_not_ready ||
          "Опція встановлення зараз недоступна. Зайдіть пізніше або перевірте, що сторінка відкрита через HTTPS.",
        5000,
      );
      return;
    }
    ...
    if (outcome !== "accepted") {
      this.toastInfo(
        t.install_dismissed ||
          "Скасовано — можна встановити пізніше з цього ж екрана.",
      );
    }
  ...
  updateThemeToggleUI(mode) {
    ...
    if (labelEl)
      labelEl.textContent = `${t.theme || "Theme"}: ${labels[mode] || labels.auto}`;
  ...
  showCreateVoting() {
    ...
    select.innerHTML =
      ...
      this.state.groups
        .map(
          (g) => `<option value="${g.id}">${this.escapeHTML(g.name)}</option>`,
        )
        .join("");
        ...
        if (group) {
          ...
          targetSelect.innerHTML =
            ...
            eligibleMembers
              .map(
                (m) =>
                  `<option value="${m.id}">${this.escapeHTML(m.name)} (${this.escapeHTML(m.address)})</option>`,
              )
              .join("");
        }
      }
  ...
  async createVoting() {
    ...
    if (!group.isAdmin) {
      ...
      if (lastVotingTime) {
        const hoursSinceLastVoting =
          (Date.now() - lastVotingTime) / (1000 * 60 * 60);
    ...
    if (
      (type === "admin-change" || type === "remove-member") &&
    ...
    if (type === "remove-member") {
      ...
      removalReason =
        reasonSelect.value === "other"
          ? reasonText.value
          : t[`reason_${reasonSelect.value}`];
    }

    if (type === "admin-change") {
      const existingAdminChange = this.state.votings.find(
        (v) =>
          v.groupId === groupId &&
          v.type === "admin-change" &&
          v.status === "active",
    ...
    if (type === "freeze") {
      ...
      if (
        !this.state.freezeSelectedMembers ||
        this.state.freezeSelectedMembers.length === 0
      ) {
    ...
    if (type === "delete-group") {
      const existingDeleteGroup = this.state.votings.find(
        (v) =>
          v.groupId === groupId &&
          v.type === "delete-group" &&
          v.status === "active",
    ...
    try {
      ...
      const targetMember = targetMemberId
        ? group.members.find((m) => m.id === targetMemberId)
        : null;
      const userName =
        [this.state.user.firstName, this.state.user.lastName]
          .filter(Boolean)
      ...
      const newVoting = {
        ...
        freezeMembers:
          type === "freeze"
            ? this.state.freezeSelectedMembers.map((m) => ({
                id: m.id,
                name: m.name,
                address: m.address,
              }))
  ...
  async joinGroup() {
    ...
    if (!code || code.length !== 6) {
      this.toastError(
        t.enter_group_id_error || "Введіть коректний ID групи (6 цифр)",
      );
      return;
    }
    ...
    try {
      // Find group by code
      const { data: group, error: findErr } =
        await supabaseService.findGroupByCode(code);
      if (findErr || !group) {
      ...
      const { data: requestId, error: reqErr } =
        await supabaseService.submitJoinRequestV2(
          group.id,
          apartment,
          asObserver,
        );
      if (reqErr) {
        ...
        if (msg.includes("already_member")) {
          ...
          const takenName = msg.includes("apartment_taken:")
            ? msg.split("apartment_taken:")[1] || ""
            : "";
          const who = takenName ? `: ${this.escapeHTML(takenName)}` : "";
          this.toastError(
            `${t.apartment_taken || "Квартира зайнята голосуючим"}${who}. ${t.apartment_taken_hint || 'Оберіть роль "спостерігач" або введіть іншу квартиру.'}`,
          );
        } else if (msg.includes("apartment_required")) {
          this.toastError(
            t.join_apartment_required || "Введіть номер квартири",
          );
        } else {
          throw new Error(msg);
        }
  ...
  async showVotingDetail(votingId) {
      ...
      if (votesRes.data) {
        voting.comments = votesRes.data
          ...
          .map((v) => ({
            userId: v.user_id,
            userName: v.voter
              ? `${v.voter.first_name} ${v.voter.last_name}`.trim()
              : "",
        ...
        const myVote = votesRes.data.find(
          (v) => v.user_id === this.state.user.id,
        );
    ...
    const dvCurrentMember = dvGroup?.members?.find(
    ...
    if (voting.comments && voting.comments.length > 0) {
      const commentsList = voting.comments
        .map((c) => {
          const voteLabel =
            c.vote === "yes"
              ? t.vote_yes
          ...
          const voteEmoji =
            c.vote === "yes"
              ? '<i class="ph-fill ph-check-circle text-success" aria-hidden="true"></i>'
              : c.vote === "no"
                ? '<i class="ph-fill ph-x-circle text-danger" aria-hidden="true"></i>'
                : '<i class="ph-fill ph-minus-circle text-muted" aria-hidden="true"></i>';
          return `
    ...
    const hasObjected =
      voting.objections &&
      voting.objections.some((o) => o.userId === this.state.user.id);
    ...
    if (isFreeze) {
      ...
      const freezeMembersChips = voting.freezeMembers
        ? voting.freezeMembers
            .map(
              (m) =>
      ...
      if (voting.objections && voting.objections.length > 0) {
        objectionsList = voting.objections
          .map(
            (o) =>
  ...
  async printProtocol(votingId) {
    ...
    const fmt = (d) => {
      if (!d) return "—";
      const dt = new Date(d);
      return isNaN(dt) ? "—" : dt.toLocaleString(locale);
    };
    const isSecret = voting.type === "secret";
    const typeLabel = isSecret
      ? t.type_secret
      : voting.type === "admin-change"
        ? t.type_admin
    ...
    const voteWord = (ch) =>
      ch === "yes" ? t.vote_yes : ch === "no" ? t.vote_no : t.vote_abstain;
    const voteColor = (ch) =>
      ch === "yes" ? "#1d7a4d" : ch === "no" ? "#b3261e" : "#5f5e5a";

    const metaRow = (label, value) =>
      `<tr><td style="color:#666;padding:3px 0;width:150px;vertical-align:top;">${esc(label)}</td><td style="padding:3px 0;font-weight:500;">${value}</td></tr>`;
    let targetRow = "";
    if (
      (voting.type === "admin-change" || voting.type === "remove-member") &&
      voting.targetMemberName
    ) {
      targetRow = metaRow(
        voting.type === "admin-change"
          ? t.target_admin_candidate
  ...
  async vote(votingId, voteType) {
    ...
    const currentMember = vGroup?.members?.find(
      (m) => m.id === this.state.user.id,
    );
    ...
    const comment = commentField
      ? commentField.value.trim().substring(0, 500)
      : "";
    ...
    try {
      const { data, error } = await supabaseService.castVote(
        votingId,
        choice,
        comment,
      );
  ...
  showDeleteVotingModal(votingId) {
    ...
    reasonField.oninput = function () {
      document.getElementById("delete-reason-counter").textContent =
        this.value.length;
    };
  ...
  async confirmDeleteVoting() {
    ...
    if (reason.length > 200) {
      this.toastError(
        t.delete_reason_long || "Причина занадто довга (макс. 200 символів)",
      );
      return;
    }
  ...
  async showGroupDetail(groupId) {
    ...
    document.getElementById("group-detail-description").textContent =
      group.description || "";
    ...
    try {
      const { data: votesData } = await supabaseService.getGroupMemberVotes(
        group.id,
      );
      group.memberVotes = {};
    ...
    const acceptedVotings = groupVotings.filter(
      (v) => v.status === "completed" && v.result === "accepted",
    ).length;
    const rejectedVotings = groupVotings.filter(
      (v) => v.status === "completed" && v.result === "rejected",
    ).length;
    const activeVotings = groupVotings.filter(
      (v) => v.status === "active",
    ).length;
    ...
    document.getElementById("group-admin-badge").style.display = group.isAdmin
      ? "inline-block"
      : "none";
  ...
  async adminRestoreMember(groupId, userId) {
    const t = this.translations[this.currentLanguage];
    const { error } = await supabaseService.setMemberFrozen(
      groupId,
      userId,
      false,
    );
  ...
  renderMembersList(group) {
    ...
    if (this.state.membersFilter && this.state.membersFilter.length >= 3) {
      ...
      membersWithStats = membersWithStats.filter(
        (m) =>
    ...
    if (this.state.membersSort.by === "name") {
      membersWithStats.sort((a, b) => {
        ...
        return this.state.membersSort.order === "asc"
          ? comparison
          : -comparison;
      });
      ...
      membersWithStats.sort((a, b) => {
        const comparison =
          a.participation.participated - b.participation.participated;
        return this.state.membersSort.order === "asc"
          ? comparison
          : -comparison;
      });
    ...
    const participationSortIcon = document.getElementById(
      "participation-sort-icon",
    );

    if (nameSortIcon) {
      nameSortIcon.className =
    ...
    if (participationSortIcon) {
      participationSortIcon.className =
    ...
    membersList.innerHTML = membersWithStats
      .map((member) => {
        const participationText = `${member.participation.participated}/${member.participation.total}`;
        const frozenIndicator = member.frozen
          ? `<i class="ph-fill ph-snowflake frozen-indicator" title="${t.frozen_badge}" aria-hidden="true"></i>`
          : "";
        ...
        const restoreBtn =
          isAdmin && member.frozen
            ...
            : "";
        const adminMenu =
          (isAdmin && member.id !== this.state.user.id
    ...
    if (group.isAdmin && group.requests.length > 0) {
      requestsList.innerHTML = group.requests
        .map((request) => {
  ...
  async exportGroupHistory() {
    ...
    if (groupVotings.length === 0) {
      this.toastError(
        (this.translations[this.currentLanguage] || {}).export_nothing ||
          "Немає голосувань для експорту",
      );
      return;
    }
    ...
    (group.members || []).forEach((m) => {
      if (m.apartment) aptByUser[m.id] = m.apartment;
    });
  ...
  sortMembersByName() {
    if (this.state.membersSort.by === "name") {
      this.state.membersSort.order =
        this.state.membersSort.order === "asc" ? "desc" : "asc";
    } else {
      this.state.membersSort.by = "name";
      this.state.membersSort.order = "asc";
    }
    const group = this.state.groups.find(
      (g) => g.id === this.state.currentGroupId,
    );
  ...
  async adminChangeRole(groupId, userId, makeObserver) {
    const t = this.translations[this.currentLanguage] || {};
    try {
      const { error } = await supabaseService.adminChangeRole(
        groupId,
        userId,
        makeObserver,
      );
  ...
  async requestRoleChange(groupId, becomeObserver) {
    const t = this.translations[this.currentLanguage] || {};
    try {
      const { error } = await supabaseService.requestRoleChange(
        groupId,
        becomeObserver,
      );
      if (error) {
        ...
        if (msg.includes("already_in_role")) {
          ...
          this.toastError(
            t.admin_cannot_be_observer ||
              "Адміністратор не може бути спостерігачем",
          );
        } else if (msg.includes("apartment_taken")) {
          const takenName = msg.includes("apartment_taken:")
            ? msg.split("apartment_taken:")[1] || ""
          ...
          const who = takenName ? `: ${this.escapeHTML(takenName)}` : "";
          this.toastError(
            `${t.apartment_taken || "Квартира зайнята голосуючим"}${who}`,
          );
        } else {
          throw new Error(msg);
        }
      ...
      this.toastSuccess(
        t.role_change_requested || "Запит на зміну ролі надіслано",
      );
    } catch (err) {
      this.toastError(this.humanError(err));
    }
  ...
  async saveEditedGroup() {
    ...
    try {
      const { error } = await supabaseService.updateGroup(groupId, {
        name,
        description,
      });
      if (error) throw new Error(error.message);
  ...
  async deleteGroup() {
    ...
    const btn = document.querySelector(
      "#delete-group-modal .btn-primary, #delete-group-modal .btn[style]",
    );
    if (btn) {
      btn.classList.add("btn-loading");
      btn.disabled = true;
  ...
  async confirmLeaveGroup() {
    ...
    const btn = document.querySelector(
      "#leave-group-modal .btn-primary, #leave-group-modal .btn[style]",
    );
    if (btn) {
      btn.classList.add("btn-loading");
      btn.disabled = true;
  ...
  updateProfileDisplay() {
    ...
    if (rolesContainer && this.state.groups.length > 0) {
      rolesContainer.innerHTML = this.state.groups
        .map((group) => {
          ...
          const isObserver =
            group.myIsObserver === true || myMember?.isObserver === true;
  ...
  translations: {
    uk: {
      ...
      install_ios_hint:
        "На iPhone/iPad: натисніть «Поділитися» (квадрат зі стрілкою) і виберіть «На екран Домівка».",
      install_not_ready:
        "Опція встановлення поки недоступна. Перевірте, що сторінку відкрито через HTTPS — і спробуйте знову.",
      install_dismissed:
        "Скасовано — можна встановити пізніше з цього ж екрана.",
      ...
      feedback_hint:
        "Напишіть, що подобається або що варто покращити. Ми читаємо все.",
      ...
      archive_confirm_msg:
        "Перенести сповіщення в архів? Вони залишаться в історії, але список очиститься.",
      archive_empty: "Список вже порожній",
      archive_done: "Перенесено в архів",
      archive_needs_migration:
        "Спочатку накатіть phase12-notif-archive.sql у Supabase SQL Editor",
      ...
      cta_complete_profile:
        "Заповніть «Квартиру/офіс» у профілі, щоб голосувати",
      ...
      group_hint:
        "Після створення ви отримаєте унікальний ID для запрошення учасників",
      create: "Створити",
      ...
      freeze_info:
        "Виключає учасника з підрахунку голосів (напр., квартиру продано або людина не бере участі). На заперечення — 5 днів; будь-які 2 учасники можуть скасувати.",
      freeze_proposal: "Пропозиція виключити з підрахунку",
      freeze_duration_info:
        "Заперечити можна 5 днів. Якщо 2 учасники натиснуть «Не згоден» — виключення скасується. Сам учасник може повернути себе будь-коли кнопкою «Я тут».",
      freeze_voting: "Виключення з підрахунку",
      only_admin_can_freeze:
        "Тільки адміністратор може запропонувати виключення з підрахунку",
      select_freeze_members: "Виберіть хоча б одного учасника",
      i_disagree: "Я не згоден",
      disagree_info:
        "Якщо 2 учасники не згодні — виключення буде скасовано автоматично.",
      ...
      frozen_cannot_vote:
        "Вас виключено з підрахунку. Натисніть «Я тут», щоб повернутися й голосувати.",
      excluded_self_text:
        "Вас виключено з підрахунку голосів цієї групи. Якщо ви берете участь — поверніть себе одним дотиком.",
      im_here_btn: "Я тут",
      ...
      protocol_secret_note:
        "Таємне голосування — поіменний список не розкривається.",
      ...
      instr_protocol_desc:
        "Будь-який учасник може роздрукувати протокол завершеного голосування. Відкрийте голосування → «Протокол» → «Друк / Зберегти PDF». У протоколі: питання, тип, період, кворум і явка, результат із підрахунком «За/Проти/Утрималися», поіменний список голосів (для відкритого голосування) та місце для підписів. Для таємного — лише підсумкові цифри, без імен.",
      duration: "Тривалість",
      hour: "година",
      hours: "години",
      days: "дні",
      ...
      empty_groups_hint:
        "Створіть свою групу або приєднайтеся за 6-значним кодом, який вам надішле адміністратор.",
      ...
      empty_votings_hint:
        "Тут з'являться голосування з ваших груп. Створіть перше — натисніть «+» вгорі.",
      ...
      instr_qs_step1:
        "1. <strong>Зареєструйтесь</strong> через email і пароль (або увійдіть, якщо акаунт уже є). Якщо забули пароль — натисніть «Забули пароль?» на екрані входу, на пошту прийде посилання для скидання.",
      instr_qs_step2:
        "2. <strong>Заповніть профіль</strong> — вкажіть ім'я, прізвище, телефон та номер квартири/ділянки (обов'язково для голосування)",
      instr_qs_step3:
        "3. <strong>Створіть групу</strong> або <strong>приєднайтесь</strong> до існуючої за 6-значним кодом (вкажіть квартиру і роль)",
      instr_qs_step4:
        "4. <strong>Голосуйте</strong> у активних голосуваннях або створюйте власні",
      instr_qs_step5:
        "5. Слідкуйте за результатами у вкладці <strong>Сповіщення</strong>",
      instr_profile: "Профіль",
      instr_profile_setup_title: "Налаштування профілю",
      instr_profile_setup_desc:
        "Після входу заповніть обов'язкові поля: ім'я, прізвище, телефон, адресу та номер квартири/ділянки. Без номера квартири ви не зможете голосувати.",
      instr_profile_edit_title: "Редагування профілю",
      instr_profile_edit_desc:
        "Відкрийте вкладку «Профіль» → натисніть «Редагувати профіль». Можна змінити будь-яке поле в будь-який час.",
      instr_profile_lang_title: "Зміна мови",
      instr_profile_lang_desc:
        "У вкладці «Профіль» виберіть мову: українська, English або русский. Вибір зберігається автоматично.",
      instr_group_mgmt: "Групи",
      instr_create_group_title: "Створення групи",
      instr_create_group_desc:
        "Натисніть «+» у вкладці «Групи». Вкажіть назву та опис. Система створить унікальний 6-значний код. Ви автоматично станете адміністратором.",
      instr_join_group_title: "Вступ до групи",
      instr_join_group_desc:
        "У вкладці «Групи» розгорніть «Вступити до групи». Введіть 6-значний код, номер своєї квартири/офісу/будинку та оберіть роль (голосуючий чи спостерігач). Натисніть «Приєднатися» — адміністратор отримає запит і має його схвалити або відхилити.",
      instr_group_detail_title: "Сторінка групи",
      instr_group_detail_desc:
      ...
      instr_delete_group_admin_desc:
        "Адміністратор може видалити групу напряму тільки якщо він єдиний учасник. Якщо в групі 2+ учасників — потрібно створити голосування «Видалення групи».",
      ...
      instr_delete_vote_desc:
        "Автор голосування може його видалити, вказавши причину (мін. 5 символів). Видалене голосування позначається відповідно.",
      ...
      instr_badge_red:
        "Відхилено (більшість проголосувала «Проти» або недостатньо голосів)",
      ...
      instr_duration_2:
        "• Зміна адміна / Видалення учасника: фіксовано 72 години",
      instr_duration_3: "• Виключення з підрахунку: 5 днів на заперечення",
      instr_decision_title: "Прийняття рішень",
      instr_decision_desc:
        "Для прийняття рішення потрібно 50%+1 голос від <strong>голосуючих</strong> учасників групи (спостерігачі не враховуються). Результат визначається автоматично по завершенню терміну.",
      instr_limits_title: "Обмеження",
      instr_limits_desc:
        "• Без номера квартири/ділянки — голосувати не можна<br>• Звичайний учасник: макс. 1 голосування на день у кожній групі<br>• Адміністратор: без обмежень<br>• Зміна адміна / видалення: мін. 3 учасники в групі",
      instr_export: "Експорт даних",
      instr_export_desc:
        "Адміністратор може завантажити історію голосувань групи у CSV-файл. У файлі: дата, автор, питання, тип, результат, кількість голосів «За/Проти/Утримався», коментарі. Файл відкривається в Excel або Google Sheets.",
      instr_roles: "Ролі: голосуючий і спостерігач",
      instr_roles_apartment_title: "Номер квартири = один голос",
      instr_roles_apartment_desc:
        "При вступі ви вказуєте номер своєї квартири / офісу / будинку. Правило просте: <strong>одна квартира — один голос</strong>. Голосувати від цієї квартири може лише одна людина (голосуючий), решта мешканців можуть вступити як спостерігачі.",
      instr_roles_voter_title: "👍 Голосуючий",
      instr_roles_voter_desc:
        "Повноцінний учасник: бачить голосування, голосує «За/Проти/Утримуюсь», створює власні голосування. Його голос рахується у результаті.",
      instr_roles_observer_title: "👁️ Спостерігач",
      instr_roles_observer_desc:
        "Бачить усі голосування та результати, але <strong>не голосує</strong>. Спостерігачі не враховуються при підрахунку (кворум рахується тільки серед голосуючих). Підходить для членів сім'ї, орендарів чи тих, хто хоче лише стежити.",
      instr_roles_change_title: "Як змінити свою роль",
      instr_roles_change_desc:
        "Відкрийте вкладку «Профіль» → у списку «Мої ролі в групах» натисніть «Запросити зміну ролі» біля потрібної групи. Запит піде адміністратору на підтвердження. Якщо у вашій квартирі вже є голосуючий — стати голосуючим не вийде, поки місце зайняте.",
      instr_roles_admin_title: "Зміна ролі адміністратором",
      instr_roles_admin_desc:
        "Адміністратор може сам змінити роль учасника: на сторінці групи у списку учасників натисніть кнопку зміни ролі (↔) біля потрібної людини.",
      instr_notif_archive_title: "Архів сповіщень",
      instr_notif_archive_desc:
        "Кнопка «В архів» ховає всі поточні сповіщення зі списку (вони не видаляються, а зберігаються). Щоб переглянути їх, натисніть значок архіву (вгорі на вкладці «Сповіщення»). В архіві є пошук від 3 літер, а кнопка повернення (↩) повертає сповіщення назад до основного списку.",
      instr_profile_password_title: "Зміна пароля",
      instr_profile_password_desc:
        "У вкладці «Профіль» натисніть «Змінити пароль». Введіть новий пароль двічі — і він одразу почне діяти.",
      instr_profile_theme_title: "Тема оформлення",
      instr_profile_theme_desc:
        "Кнопка «Тема» у профілі перемикає вигляд: світла, темна або системна (автоматично за налаштуваннями телефону). Вибір зберігається.",
      instr_profile_install_title: "Встановити на телефон",
      instr_profile_install_desc:
        "Якщо з'явилася кнопка «Встановити на головний екран» — натисніть її, і Spilka додасться як застосунок (окрема іконка, працює як звичайна програма, без браузера).",
      instr_profile_feedback_title: "Пропозиція або зауваження",
      instr_profile_feedback_desc:
      ...
      delete_voting_warning:
        "⚠️ Голосування буде видалено безповоротно. Всі учасники отримають повідомлення.",
      ...
      daily_limit_reached:
        "Ви досягли ліміту: 1 голосування на 24 години для звичайних користувачів",
      abstain: "Утриматися",
      comment: "Коментар",
      comment_placeholder:
      ...
      terms_intro:
        "Коротко про головне — будь ласка, прочитайте перед початком:",
      terms_s_data:
        "Ми зберігаємо ваші дані: email, ім'я та прізвище, телефон, адресу, номер квартири/ділянки, ваші голоси й коментарі. Використовуємо їх лише для роботи голосувань — нікому не передаємо і не продаємо.",
      terms_s_visibility:
        "Учасники вашої групи бачать ваше ім'я, номер квартири/ділянки та ваші голоси (крім таємних голосувань). Будь-який учасник може завантажити історію голосувань групи.",
      terms_s_asis:
        "Сервіс безкоштовний і надається «як є», без гарантій безперебійної роботи. Можливі збої або втрата даних — важливі результати зберігайте собі («Протокол» або «Експорт CSV»).",
      terms_s_security:
        "Ми захищаємо дані (шифрування паролів, обмежений доступ, резервні копії), але жодна система в інтернеті не захищена на 100%. Не використовуйте тут пароль від пошти чи банку.",
      terms_s_legal:
        "Голосування у Спілці — допоміжний інструмент. Вони не замінюють офіційні збори (ОСББ, кооперативу тощо) і не мають самостійної юридичної сили.",
      terms_s_rights:
        "Ви можете будь-коли вийти з групи або попросити видалити ваш акаунт і всі дані через форму зворотного зв'язку у профілі. Сервіс — для осіб від 18 років.",
      terms_s_changes:
        "Умови можуть оновлюватися — при суттєвих змінах ми покажемо їх знову і попросимо погодитися ще раз.",
      ...
      terms_agree_text:
        "Я приймаю умови використання і даю згоду на обробку моїх персональних даних",
      terms_accept_error:
        "Не вдалося зберегти згоду. Перевірте інтернет і спробуйте ще раз.",
      terms_full_1_title: "Що таке Спілка",
      terms_full_1_body:
        "Спілка (spilka.top) — онлайн-застосунок для голосувань у житлових спільнотах: ОСББ, кооперативах, садових товариствах, будинкових комітетах. Реєструючись, ви погоджуєтесь користуватися сервісом на умовах, описаних нижче.",
      ...
      leave: "Покинути",
      admin_cannot_leave:
        "Адміністратор не може покинути групу. Спочатку передайте роль адміністратора іншому учаснику.",
      delete_group_need_voting:
        "У групі є інші учасники. Для видалення групи створіть голосування типу «Видалення групи».",
      type_delete_group: "Видалення групи",
      group_deleted_by_voting: "Групу видалено за результатами голосування",
      delete_group_warning:
        "Якщо голосування буде прийнято — групу буде видалено автоматично разом з усіма голосуваннями та історією.",
      one_delete_group_at_time:
        "Вже існує активне голосування за видалення цієї групи.",
      min_duration_24h:
      ...
      apartment_taken_hint:
        'Оберіть роль "спостерігач" або введіть іншу квартиру.',
      ...
      err_exclusion_only_via_voting:
        "Виключення можливе лише через голосування",
    ...
    en: {
      ...
      install_ios_hint:
        'On iPhone/iPad: tap "Share" (square with arrow) and choose "Add to Home Screen".',
      install_not_ready:
        "Install option is not available right now. Make sure you opened the page via HTTPS and try again.",
      ...
      feedback_hint:
        "Tell us what works well or what to improve. We read every message.",
      ...
      archive_confirm_msg:
        "Move notifications to archive? They stay in history but the list is cleared.",
      archive_empty: "List is already empty",
      archive_done: "Moved to archive",
      archive_needs_migration:
        "Apply phase12-notif-archive.sql in Supabase SQL Editor first",
      ...
      cta_complete_profile:
        'Fill in "Apartment/office" in your profile to vote',
      ...
      group_hint:
        "After creation you will receive a unique ID to invite members",
      create: "Create",
      ...
      freeze_info:
        "Excludes a member from the vote count (e.g. the flat was sold or the person does not take part). 5 days to object; any 2 members can cancel it.",
      freeze_proposal: "Proposal to exclude from the count",
      freeze_duration_info:
        'You have 5 days to object. If 2 members click "I disagree" the exclusion is cancelled. The member can return themselves any time with "I\'m here".',
      freeze_voting: "Exclude from count",
      only_admin_can_freeze:
        "Only the administrator can propose excluding a member from the count",
      select_freeze_members: "Select at least one member",
      i_disagree: "I disagree",
      disagree_info:
        "If 2 members disagree the exclusion is cancelled automatically.",
      ...
      excluded_self_text:
        "You have been excluded from this group's vote count. If you take part, return yourself with one tap.",
      ...
      protocol_secret_note:
        "Secret ballot — the itemized list is not disclosed.",
      ...
      instr_protocol_desc:
        'Any member can print the protocol of a completed voting. Open the voting → "Protocol" → "Print / Save PDF". The protocol contains: question, type, period, quorum and turnout, the result with Yes/No/Abstain tallies, an itemized list of votes (for open voting) and space for signatures. For a secret ballot only the summary figures are shown, without names.',
      duration: "Duration",
      hour: "hour",
      hours: "hours",
      days: "days",
      ...
      empty_groups_hint:
        "Create your own group or join one using the 6-digit code your admin sent you.",
      ...
      empty_votings_hint:
        'Votings from your groups will appear here. Tap the "+" button above to create the first one.',
      ...
      instr_qs_step1:
        '1. <strong>Sign up</strong> with email and password (or sign in if you already have an account). Forgot your password? Tap "Forgot your password?" on the sign-in screen and you\'ll get a reset link by email.',
      instr_qs_step2:
        "2. <strong>Complete your profile</strong> — enter name, phone, and apartment/plot number (required to vote)",
      instr_qs_step3:
        "3. <strong>Create a group</strong> or <strong>join</strong> an existing one using the 6-digit code (enter your apartment and role)",
      instr_qs_step4:
        "4. <strong>Vote</strong> in active votings or create your own",
      instr_qs_step5:
        "5. Track results in the <strong>Notifications</strong> tab",
      instr_profile: "Profile",
      instr_profile_setup_title: "Profile Setup",
      instr_profile_setup_desc:
        "After signing in, fill in required fields: name, surname, phone, address, and apartment/plot number. Without an apartment number, you cannot vote.",
      instr_profile_edit_title: "Edit Profile",
      instr_profile_edit_desc:
        'Open the "Profile" tab → tap "Edit Profile". You can change any field at any time.',
      instr_profile_lang_title: "Change Language",
      instr_profile_lang_desc:
        'In the "Profile" tab, select your language: Ukrainian, English, or Russian. Your choice is saved automatically.',
      instr_group_mgmt: "Groups",
      instr_create_group_title: "Creating a Group",
      instr_create_group_desc:
        'Tap "+" in the "Groups" tab. Enter a name and description. The system will generate a unique 6-digit code. You automatically become the administrator.',
      instr_join_group_title: "Joining a Group",
      instr_join_group_desc:
        'On the "Groups" tab, expand "Join a group". Enter the 6-digit code, your apartment / office / house number and choose a role (voter or observer). Tap "Join" — the administrator will receive a request and must approve or reject it.',
      ...
      instr_delete_group_admin_desc:
        'The administrator can delete the group directly only if they are the only member. If there are 2+ members — a "Delete Group" voting must be created.',
      ...
      instr_delete_vote_desc:
        "The voting author can delete it by providing a reason (min. 5 characters). Deleted votings are marked accordingly.",
      instr_comments_title: "Comments",
      instr_comments_desc:
        "You can add a comment when voting in open votings (up to 500 characters). In secret votings, comments are hidden.",
      instr_notifications: "Notifications",
      instr_notif_desc:
        "You receive notifications about: new votings, join requests, voting results, admin changes, member removal, and freezing. Unread ones are marked blue. You can mark all as read.",
      ...
      instr_decision_desc:
        "A 50%+1 vote from the group's <strong>voters</strong> is required for any decision (observers are not counted). Results are determined automatically when the time expires.",
      instr_limits_title: "Limits",
      instr_limits_desc:
        "• Without apartment/plot number — you cannot vote<br>• Regular member: max 1 voting per day per group<br>• Administrator: no limits<br>• Change admin / removal: min. 3 members in group",
      instr_export: "Data Export",
      instr_export_desc:
        "The administrator can download the group's voting history as a CSV file. Contents: date, author, question, type, result, vote counts (Yes/No/Abstain), comments. Opens in Excel or Google Sheets.",
      instr_roles: "Roles: voter and observer",
      instr_roles_apartment_title: "One apartment = one vote",
      instr_roles_apartment_desc:
        "When joining you enter your apartment / office / house number. The rule is simple: <strong>one apartment — one vote</strong>. Only one person per apartment can vote (the voter); other residents can join as observers.",
      instr_roles_voter_title: "👍 Voter",
      instr_roles_voter_desc:
        'A full member: sees votings, casts "Yes/No/Abstain", creates their own votings. Their vote counts toward the result.',
      instr_roles_observer_title: "👁️ Observer",
      instr_roles_observer_desc:
        "Sees all votings and results but <strong>does not vote</strong>. Observers are not counted (quorum is calculated among voters only). Good for family members, tenants, or anyone who just wants to follow along.",
      instr_roles_change_title: "How to change your role",
      instr_roles_change_desc:
        'Open the "Profile" tab → in "My roles in groups" tap "Request role change" next to the group. The request goes to the administrator for approval. If your apartment already has a voter, you can\'t become a voter until that seat is free.',
      instr_roles_admin_title: "Role change by the administrator",
      instr_roles_admin_desc:
        "The administrator can change a member's role directly: on the group page, in the members list, tap the change-role button (↔) next to the person.",
      instr_notif_archive_title: "Notification archive",
      instr_notif_archive_desc:
        'The "Archive" button hides all current notifications from the list (they are kept, not deleted). To view them, tap the archive icon (top of the "Notifications" tab). The archive has search from 3 letters, and the return button (↩) sends a notification back to the main list.',
      instr_profile_password_title: "Change password",
      instr_profile_password_desc:
        'On the "Profile" tab tap "Change password". Enter the new password twice — it takes effect immediately.',
      instr_profile_theme_title: "Appearance theme",
      instr_profile_theme_desc:
        'The "Theme" button in the profile switches the look: light, dark, or system (automatically following your phone settings). The choice is saved.',
      instr_profile_install_title: "Install on your phone",
      instr_profile_install_desc:
        'If an "Add to home screen" button appears — tap it, and Spilka will be added as an app (its own icon, works like a regular app, without the browser).',
      ...
      delete_voting_warning:
        "⚠️ Voting will be permanently deleted. All members will be notified.",
      ...
      daily_limit_reached:
        "You have reached the limit: 1 voting per 24 hours for regular users",
      abstain: "Abstain",
      comment: "Comment",
      ...
      terms_s_data:
        "We store your data: email, first and last name, phone, address, apartment/plot number, your votes and comments. We use it only to run the votings — we never share or sell it.",
      terms_s_visibility:
        "Members of your group see your name, apartment/plot number and your votes (except secret ballots). Any member can download the group voting history.",
      terms_s_asis:
        'The service is free and provided "as is", with no uptime guarantees. Failures or data loss are possible — keep important results yourself ("Protocol" or "CSV export").',
      terms_s_security:
        "We protect the data (password encryption, restricted access, backups), but no online system is 100% secure. Do not reuse your email or bank password here.",
      terms_s_legal:
        "Votings in Spilka are a supporting tool. They do not replace official meetings (HOA, co-op etc.) and have no legal force on their own.",
      terms_s_rights:
        "You can leave a group at any time or ask us to delete your account and all data via the feedback form in your profile. The service is for persons 18+.",
      terms_s_changes:
        "The terms may be updated — on material changes we will show them again and ask you to agree once more.",
      ...
      terms_agree_text:
        "I accept the Terms of Service and consent to the processing of my personal data",
      terms_accept_error:
        "Could not save your consent. Check your connection and try again.",
      terms_full_1_title: "What Spilka is",
      terms_full_1_body:
        "Spilka (spilka.top) is an online voting app for residential communities: HOAs, co-ops, garden societies, house committees. By registering you agree to use the service under the terms below.",
      terms_full_2_title: "What data we collect and why",
      terms_full_2_body:
        'To run the service we store: email and password (the password is encrypted), first and last name, phone (if provided), address, apartment/plot number, your votes, comments and feedback messages. This data is used solely to run votings and identify members within your group. We do not share or sell it to third parties and do not use it for advertising. By accepting these terms you consent to the processing and storage of this personal data in accordance with the Law of Ukraine "On Personal Data Protection".',
      terms_full_3_title: "Who sees your data",
      terms_full_3_body:
        "Members of your group see your name, surname, apartment/plot number and your votes in open votings. In secret ballots nobody sees your choice, including the group administrator. Any group member can download the group voting history (CSV or protocol) showing apartments and votes of open votings. The service administrator has technical access to the data — solely to keep the app running.",
      terms_full_4_title: 'The service is provided "as is"',
      terms_full_4_body:
        'The service is free. We do not guarantee uninterrupted operation: technical failures, downtime and, in extreme cases, data loss are possible. By using the service you agree that the service owner does not compensate damages related to unavailability or data loss. We recommend saving important voting results right away: the "Protocol" button (print/PDF) or "CSV export".',
      terms_full_5_title: "Security and risks",
      terms_full_5_body:
        "We take reasonable protection measures: passwords are stored encrypted, server access is restricted, backups are made regularly. Still, no online system is 100% secure — a risk of breach or leak exists. By accepting the terms you acknowledge this risk. Do not reuse your email or bank password in Spilka.",
      terms_full_6_title: "Legal status of votings",
      terms_full_6_body:
        "Spilka is a supporting tool for gauging community opinion. Votings in the app do not replace official owners' meetings required by law and have no legal force on their own. If a decision needs official formalization — holding the meeting and drawing up the minutes remains the responsibility of the community members themselves.",
      terms_full_7_title: "Your rights",
      terms_full_7_body:
        "At any time you can: leave a group; edit your profile data; ask us to fully delete your account and all data — write via the feedback form in your profile and we will delete them. The service is intended for persons aged 18 or older.",
      terms_full_8_title: "Changes to the terms",
      terms_full_8_body:
        "We may update these terms. On material changes the app will show the new version and ask you to agree again — without consent the service cannot be used. Questions about the terms — via the feedback form in your profile.",
      ...
      leave: "Leave",
      admin_cannot_leave:
        "Administrator cannot leave the group. Transfer the admin role to another member first.",
      delete_group_need_voting:
        'There are other members in the group. To delete the group, create a "Delete group" voting.',
      type_delete_group: "Delete group",
      group_deleted_by_voting: "Group deleted by voting result",
      delete_group_warning:
        "If the vote passes — the group will be automatically deleted along with all votings and history.",
      one_delete_group_at_time:
      ...
      apartment_taken_hint:
        'Choose "observer" role or enter a different apartment.',
      ...
      err_exclusion_only_via_voting:
        "Exclusion is only possible through a voting",
    ...
    ru: {
      ...
      install_ios_hint:
        "На iPhone/iPad: нажмите «Поделиться» (квадрат со стрелкой) и выберите «На экран Домой».",
      install_not_ready:
        "Опция установки сейчас недоступна. Проверьте, что страница открыта через HTTPS — и попробуйте снова.",
      ...
      feedback_hint:
        "Напишите, что нравится или что стоит улучшить. Мы читаем всё.",
      feedback_placeholder: "Ваше предложение или замечание...",
      feedback_send: "Отправить",
      feedback_thanks:
        "Спасибо! Мы получили ваш отзыв — в ближайшее время рассмотрим.",
      ...
      archive_confirm_msg:
        "Перенести уведомления в архив? Они останутся в истории, но список очистится.",
      archive_empty: "Список уже пуст",
      archive_done: "Перенесено в архив",
      archive_needs_migration:
        "Сначала накатите phase12-notif-archive.sql в Supabase SQL Editor",
      ...
      cta_complete_profile:
        "Заполните «Квартиру/офис» в профиле, чтобы голосовать",
      ...
      group_hint:
        "После создания вы получите уникальный ID для приглашения участников",
      create: "Создать",
      ...
      freeze_info:
        "Исключает участника из подсчёта голосов (например, квартиру продали или человек не участвует). На возражение — 5 дней; любые 2 участника могут отменить.",
      freeze_proposal: "Предложение исключить из подсчёта",
      freeze_duration_info:
        "Возразить можно 5 дней. Если 2 участника нажмут «Не согласен» — исключение отменяется. Сам участник может вернуть себя в любой момент кнопкой «Я тут».",
      freeze_voting: "Исключение из подсчёта",
      only_admin_can_freeze:
        "Только администратор может предложить исключение из подсчёта",
      select_freeze_members: "Выберите хотя бы одного участника",
      i_disagree: "Я не согласен",
      disagree_info:
        "Если 2 участника не согласны — исключение будет отменено автоматически.",
      ...
      excluded_self_text:
        "Вы исключены из подсчёта голосов этой группы. Если вы участвуете — верните себя одним касанием.",
      im_here_btn: "Я тут",
      ...
      protocol_secret_note:
        "Тайное голосование — поименный список не раскрывается.",
      ...
      empty_groups_hint:
        "Создайте свою группу или присоединитесь по 6-значному коду, который вам пришлёт администратор.",
      ...
      empty_votings_hint:
        "Здесь появятся голосования из ваших групп. Нажмите «+» вверху, чтобы создать первое.",
      ...
      instr_qs_step1:
        "1. <strong>Зарегистрируйтесь</strong> через email и пароль (или войдите, если аккаунт уже есть). Забыли пароль? Нажмите «Забули пароль?» на экране входа — на почту придёт ссылка для сброса.",
      instr_qs_step2:
        "2. <strong>Заполните профиль</strong> — укажите имя, фамилию, телефон и номер квартиры/участка (обязательно для голосования)",
      instr_qs_step3:
        "3. <strong>Создайте группу</strong> или <strong>присоединитесь</strong> к существующей по 6-значному коду (укажите квартиру и роль)",
      instr_qs_step4:
        "4. <strong>Голосуйте</strong> в активных голосованиях или создавайте свои",
      instr_qs_step5:
        "5. Следите за результатами во вкладке <strong>Уведомления</strong>",
      instr_profile: "Профиль",
      instr_profile_setup_title: "Настройка профиля",
      instr_profile_setup_desc:
        "После входа заполните обязательные поля: имя, фамилию, телефон, адрес и номер квартиры/участка. Без номера квартиры голосовать нельзя.",
      instr_profile_edit_title: "Редактирование профиля",
      instr_profile_edit_desc:
        "Откройте вкладку «Профиль» → нажмите «Редактировать профиль». Можно изменить любое поле в любое время.",
      instr_profile_lang_title: "Смена языка",
      instr_profile_lang_desc:
        "Во вкладке «Профиль» выберите язык: українська, English или русский. Выбор сохраняется автоматически.",
      instr_group_mgmt: "Группы",
      instr_create_group_title: "Создание группы",
      instr_create_group_desc:
        "Нажмите «+» во вкладке «Группы». Укажите название и описание. Система создаст уникальный 6-значный код. Вы автоматически станете администратором.",
      instr_join_group_title: "Вступление в группу",
      instr_join_group_desc:
        "На вкладке «Группы» разверните «Вступить в группу». Введите 6-значный код, номер своей квартиры/офиса/дома и выберите роль (голосующий или наблюдатель). Нажмите «Присоединиться» — администратор получит запрос и должен его одобрить или отклонить.",
      ...
      instr_delete_group_admin_desc:
        "Администратор может удалить группу напрямую только если он единственный участник. Если в группе 2+ участников — нужно создать голосование «Удаление группы».",
      ...
      instr_delete_vote_desc:
        "Автор голосования может его удалить, указав причину (мин. 5 символов). Удалённое голосование помечается соответственно.",
      instr_comments_title: "Комментарии",
      instr_comments_desc:
        "К открытым голосованиям можно добавить комментарий при голосовании (до 500 символов). В тайных голосованиях комментарии скрыты.",
      instr_notifications: "Уведомления",
      instr_notif_desc:
        "Вы получаете уведомления о: новых голосованиях, запросах на вступление, результатах голосований, сменах администратора, удалениях и заморозках участников. Непрочитанные отмечены синим. Можно отметить все как прочитанные.",
      ...
      instr_badge_red:
        "Отклонено (большинство проголосовало «Против» или недостаточно голосов)",
      ...
      instr_duration_2:
        "• Смена админа / Удаление участника: фиксировано 72 часа",
      instr_duration_3: "• Исключение из подсчёта: 5 дней на возражение",
      instr_decision_title: "Принятие решений",
      instr_decision_desc:
        "Для принятия решения нужно 50%+1 голос от <strong>голосующих</strong> участников группы (наблюдатели не учитываются). Результат определяется автоматически по завершении срока.",
      instr_limits_title: "Ограничения",
      instr_limits_desc:
        "• Без номера квартиры/участка — голосовать нельзя<br>• Обычный участник: макс. 1 голосование в день в каждой группе<br>• Администратор: без ограничений<br>• Смена админа / удаление: мин. 3 участника в группе",
      instr_export: "Экспорт данных",
      instr_export_desc:
        "Администратор может скачать историю голосований группы в CSV-файл. В файле: дата, автор, вопрос, тип, результат, количество голосов «За/Против/Воздержался», комментарии. Открывается в Excel или Google Sheets.",
      instr_roles: "Роли: голосующий и наблюдатель",
      instr_roles_apartment_title: "Номер квартиры = один голос",
      instr_roles_apartment_desc:
        "При вступлении вы указываете номер своей квартиры / офиса / дома. Правило простое: <strong>одна квартира — один голос</strong>. Голосовать от этой квартиры может только один человек (голосующий), остальные жильцы могут вступить как наблюдатели.",
      instr_roles_voter_title: "👍 Голосующий",
      instr_roles_voter_desc:
        "Полноценный участник: видит голосования, голосует «За/Против/Воздержусь», создаёт собственные голосования. Его голос учитывается в результате.",
      instr_roles_observer_title: "👁️ Наблюдатель",
      instr_roles_observer_desc:
        "Видит все голосования и результаты, но <strong>не голосует</strong>. Наблюдатели не учитываются при подсчёте (кворум считается только среди голосующих). Подходит для членов семьи, арендаторов или тех, кто хочет лишь следить.",
      instr_roles_change_title: "Как сменить свою роль",
      instr_roles_change_desc:
        "Откройте вкладку «Профиль» → в списке «Мои роли в группах» нажмите «Запросить смену роли» рядом с нужной группой. Запрос уйдёт администратору на подтверждение. Если в вашей квартире уже есть голосующий — стать голосующим не получится, пока место занято.",
      instr_roles_admin_title: "Смена роли администратором",
      instr_roles_admin_desc:
        "Администратор может сам сменить роль участника: на странице группы в списке участников нажмите кнопку смены роли (↔) рядом с нужным человеком.",
      instr_notif_archive_title: "Архив уведомлений",
      instr_notif_archive_desc:
        "Кнопка «В архив» прячет все текущие уведомления из списка (они не удаляются, а сохраняются). Чтобы их посмотреть, нажмите значок архива (вверху на вкладке «Уведомления»). В архиве есть поиск от 3 букв, а кнопка возврата (↩) возвращает уведомление обратно в основной список.",
      instr_profile_password_title: "Смена пароля",
      instr_profile_password_desc:
        "На вкладке «Профиль» нажмите «Изменить пароль». Введите новый пароль дважды — и он сразу начнёт действовать.",
      instr_profile_theme_title: "Тема оформления",
      instr_profile_theme_desc:
        "Кнопка «Тема» в профиле переключает вид: светлая, тёмная или системная (автоматически по настройкам телефона). Выбор сохраняется.",
      instr_profile_install_title: "Установить на телефон",
      instr_profile_install_desc:
        "Если появилась кнопка «Установить на главный экран» — нажмите её, и Spilka добавится как приложение (отдельная иконка, работает как обычная программа, без браузера).",
      ...
      delete_voting_warning:
        "⚠️ Голосование будет удалено безвозвратно. Все участники получат уведомление.",
      ...
      daily_limit_reached:
        "Вы достигли лимита: 1 голосование на 24 часа для обычных пользователей",
      ...
      terms_s_data:
        "Мы храним ваши данные: email, имя и фамилию, телефон, адрес, номер квартиры/участка, ваши голоса и комментарии. Используем их только для работы голосований — никому не передаём и не продаём.",
      terms_s_visibility:
        "Участники вашей группы видят ваше имя, номер квартиры/участка и ваши голоса (кроме тайных голосований). Любой участник может скачать историю голосований группы.",
      terms_s_asis:
        "Сервис бесплатный и предоставляется «как есть», без гарантий бесперебойной работы. Возможны сбои или потеря данных — важные результаты сохраняйте себе («Протокол» или «Экспорт CSV»).",
      terms_s_security:
        "Мы защищаем данные (шифрование паролей, ограниченный доступ, резервные копии), но ни одна система в интернете не защищена на 100%. Не используйте здесь пароль от почты или банка.",
      terms_s_legal:
        "Голосования в Спилке — вспомогательный инструмент. Они не заменяют официальные собрания (ОСМД, кооператива и т.п.) и не имеют самостоятельной юридической силы.",
      terms_s_rights:
        "Вы можете в любой момент выйти из группы или попросить удалить ваш аккаунт и все данные через форму обратной связи в профиле. Сервис — для лиц от 18 лет.",
      terms_s_changes:
        "Условия могут обновляться — при существенных изменениях мы покажем их снова и попросим согласиться ещё раз.",
      ...
      terms_full_1_body:
        "Спилка (spilka.top) — онлайн-приложение для голосований в жилых сообществах: ОСМД, кооперативах, садовых товариществах, домовых комитетах. Регистрируясь, вы соглашаетесь пользоваться сервисом на условиях, описанных ниже.",
      terms_full_2_title: "Какие данные мы собираем и зачем",
      terms_full_2_body:
        "Для работы сервиса мы храним: email и пароль (пароль — в зашифрованном виде), имя и фамилию, телефон (если указали), адрес, номер квартиры/участка, ваши голоса, комментарии и сообщения в форме обратной связи. Эти данные нужны исключительно для проведения голосований и идентификации участников в вашей группе. Мы не передаём и не продаём их третьим лицам и не используем для рекламы. Принимая условия, вы даёте согласие на обработку и хранение этих персональных данных в соответствии с Законом Украины «О защите персональных данных».",
      terms_full_3_title: "Кто видит ваши данные",
      terms_full_3_body:
        "Участники вашей группы видят ваше имя, фамилию, номер квартиры/участка и ваши голоса в открытых голосованиях. В тайных голосованиях ваш выбор не видит никто, включая администратора группы. Любой участник группы может скачать историю голосований группы (CSV или протокол), где видны квартиры и голоса открытых голосований. Технический доступ к данным имеет администратор сервиса — исключительно для поддержки работы приложения.",
      ...
      terms_full_6_body:
        "Спилка — вспомогательный инструмент для выяснения мнения сообщества. Голосования в приложении не заменяют официальные собрания совладельцев (ОСМД, ЖСК и т.п.), предусмотренные законодательством, и сами по себе не имеют юридической силы. Если решение требует официального оформления — за проведение собрания и оформление протокола отвечают сами участники сообщества.",
      terms_full_7_title: "Ваши права",
      terms_full_7_body:
        "Вы можете в любой момент: выйти из группы; изменить свои данные в профиле; попросить полностью удалить ваш аккаунт и все данные — напишите через форму обратной связи в профиле, и мы их удалим. Сервис предназначен для лиц, достигших 18 лет.",
      terms_full_8_title: "Изменение условий",
      terms_full_8_body:
        "Мы можем обновлять эти условия. При существенных изменениях приложение покажет новую версию и попросит согласиться ещё раз — без согласия пользоваться сервисом нельзя. Вопросы по условиям — через форму обратной связи в профиле.",
      ...
      leave: "Покинуть",
      admin_cannot_leave:
        "Администратор не может покинуть группу. Сначала передайте роль администратора другому участнику.",
      delete_group_need_voting:
        "В группе есть другие участники. Для удаления группы создайте голосование типа «Удаление группы».",
      type_delete_group: "Удаление группы",
      group_deleted_by_voting: "Группа удалена по результатам голосования",
      delete_group_warning:
        "Если голосование будет принято — группа будет удалена автоматически вместе со всеми голосованиями и историей.",
      one_delete_group_at_time:
        "Уже существует активное голосование за удаление этой группы.",
      min_duration_24h:
      ...
      apartment_taken_hint:
        'Выберите роль "наблюдатель" или введите другую квартиру.',
      ...
      err_request_not_found:
        "Заявка не найдена — возможно, она уже рассмотрена",
      err_exclusion_only_via_voting:
        "Исключение возможно только через голосование",
  ...
  removeFreezeMember(id) {
    this.state.freezeSelectedMembers = this.state.freezeSelectedMembers.filter(
      (m) => m.id !== id,
    );
    this.renderFreezeMemberChips();
  },
  ...
  renderFreezeMemberChips() {
    ...
    container.innerHTML = this.state.freezeSelectedMembers
      .map(
        (m) => `
  ...
  async objectToFreeze(votingId) {
    const t = this.translations[this.currentLanguage];
    const voting = this.state.votings.find((v) => v.id === votingId);
    if (!voting || voting.type !== "freeze" || voting.status !== "active")
      return;
  ...
  instructionsContent: {
    uk: {
      ...
      simple_voting_desc:
        "Відкрите голосування, де всі бачать, хто і як проголосував після свого вибору. Підходить для загальних питань ОСГ.",
      secret_voting_title: "Тайне голосування",
      secret_voting_desc:
        "Приховане голосування — після голосування ви бачите тільки загальну кількість «за» та «проти», без імен. Використовується для чутливих питань.",
      admin_change_title: "Зміна адміністратора",
      admin_change_desc:
        "Спеціальне голосування для зміни керівника групи. Вимагає мінімум 3 учасників. Триває 72 години. Для прийняття рішення потрібно 50%+1 голос. Поки йде голосування, адмін не може видаляти учасників.",
      remove_member_title: "Видалення учасника",
      remove_member_desc:
        "Голосування про виключення учасника з групи. Рішення приймається більшістю 50%+1 голос. Тривалість — 72 години.",
      group_management_title: "👥 Управління групою",
      create_group_title: "Створення групи",
      create_group_desc:
        "Будь-який користувач може створити групу. Система автоматично генерує унікальний 6-значний ID для запрошення.",
      ...
      delete_group_voting_desc:
        "Будь-який учасник може створити голосування за видалення групи. Мінімальна тривалість — 24 години. Якщо 50%+1 проголосувало «за» — група видаляється автоматично, а всі учасники отримують сповіщення.",
      leave_group_title: "Вихід із групи",
      leave_group_desc:
        "Будь-який учасник (крім адміністратора) може добровільно покинути групу через меню групи (⋮). Адміністратор повинен спочатку передати свою роль через голосування «Зміна адміністратора».",
      badges_title: "🎨 Позначення голосувань",
      badges_desc:
        "🟡 — Голосування не відбулося (менше 50%+1 голосів)\n🟢 — Прийнято «за» (більшість проголосувала позитивно)\n🔴 — Прийнято «проти» (більшість проголосувала негативно)",
      duration_title: "⏱️ Тривалість голосування",
      duration_desc:
        "• Звичайні голосування: від 1 години до 5 днів\n• Управлінські голосування (зміна адміна, видалення учасника): фіксовано 72 години\n• Видалення групи: від 24 годин\nРезультат визначається автоматично по закінченню терміну.",
      archive_title: "📋 Архівування",
      archive_desc:
        "Адміністратор може експортувати історію голосувань групи до Google Sheets. У таблиці зберігаються: дата, питання, опис, результат, кількість учасників.",
    },
    en: {
      voting_types_title: "🗳️ Voting Types",
      simple_voting_title: "Standard Voting",
      simple_voting_desc:
        "Open voting where everyone can see who voted how after making their choice. Suitable for general HOA matters.",
      secret_voting_title: "Secret Voting",
      secret_voting_desc:
        'Hidden voting — after voting you only see the total number of "for" and "against" votes, without names. Used for sensitive matters.',
      admin_change_title: "Change Administrator",
      admin_change_desc:
        "Special voting to change group leader. Requires at least 3 members. Lasts 72 hours. Decision requires 50%+1 vote. While voting is in progress, admin cannot remove members.",
      remove_member_title: "Remove Member",
      remove_member_desc:
        "Voting to exclude a member from the group. Decision is made by majority 50%+1 vote. Duration — 72 hours.",
      group_management_title: "👥 Group Management",
      create_group_title: "Creating a Group",
      create_group_desc:
        "Any user can create a group. The system automatically generates a unique 6-digit ID for invitations.",
      ...
      delete_group_voting_desc:
        'Any member can create a vote to delete the group. Minimum duration is 24 hours. If 50%+1 vote "yes" — the group is automatically deleted and all members are notified.',
      leave_group_title: "Leaving a Group",
      leave_group_desc:
        'Any member (except the administrator) can voluntarily leave the group via the group menu (⋮). The administrator must first transfer their role through a "Change Administrator" voting.',
      badges_title: "🎨 Voting Badges",
      badges_desc:
        '🟡 — Voting did not take place (less than 50%+1 votes)\n🟢 — Accepted "for" (majority voted positively)\n🔴 — Accepted "against" (majority voted negatively)',
      duration_title: "⏱️ Voting Duration",
      duration_desc:
        "• Standard voting: from 1 hour to 5 days\n• Administrative voting (change admin, member removal): fixed 72 hours\n• Group deletion: from 24 hours\nResult is determined automatically at the end of the term.",
      archive_title: "📋 Archiving",
      archive_desc:
        "The administrator can export the group's voting history to Google Sheets. The table contains: date, question, description, result, number of participants.",
    },
    ru: {
      voting_types_title: "🗳️ Типы голосования",
      simple_voting_title: "Обычное голосование",
      simple_voting_desc:
        "Открытое голосование, где все видят, кто и как проголосовал после своего выбора. Подходит для общих вопросов ОСГ.",
      secret_voting_title: "Тайное голосование",
      secret_voting_desc:
        "Скрытое голосование — после голосования вы видите только общее количество «за» и «против», без имён. Используется для чувствительных вопросов.",
      admin_change_title: "Смена администратора",
      admin_change_desc:
        "Специальное голосование для смены руководителя группы. Требуется минимум 3 участника. Длится 72 часа. Для принятия решения нужно 50%+1 голос. Пока идёт голосование, админ не может удалять участников.",
      remove_member_title: "Удаление участника",
      remove_member_desc:
        "Голосование об исключении участника из группы. Решение принимается большинством 50%+1 голос. Длительность — 72 часа.",
      group_management_title: "👥 Управление группой",
      create_group_title: "Создание группы",
      create_group_desc:
        "Любой пользователь может создать группу. Система автоматически генерирует уникальный 6-значный ID для приглашения.",
      ...
      delete_group_voting_desc:
        "Любой участник может создать голосование за удаление группы. Минимальная длительность — 24 часа. Если 50%+1 проголосовало «за» — группа удаляется автоматически, а все участники получают уведомление.",
      leave_group_title: "Выход из группы",
      leave_group_desc:
        "Любой участник (кроме администратора) может добровольно покинуть группу через меню группы (⋮). Администратор должен сначала передать свою роль через голосование «Смена администратора».",
      badges_title: "🎨 Обозначения голосований",
      badges_desc:
        "🟡 — Голосование не состоялось (меньше 50%+1 голосов)\n🟢 — Принято «за» (большинство проголосовало положительно)\n🔴 — Принято «против» (большинство проголосовало отрицательно)",
      duration_title: "⏱️ Длительность голосования",
      duration_desc:
        "• Обычные голосования: от 1 часа до 5 дней\n• Административные голосования (смена админа, удаление участника): фиксировано 72 часа\n• Удаление группы: от 24 часов\nРезультат определяется автоматически по окончании срока.",
      archive_title: "📋 Архивирование",
      archive_desc:
        "Администратор может экспортировать историю голосований группы в Google Sheets. В таблице сохраняются: дата, вопрос, описание, результат, количество участников.",
    },
  },
