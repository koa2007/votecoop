module.exports = {
  genCode: function (app) {
    for (let i = 0; i < 30; i++) {
      let c = "";
      for (let j = 0; j < 6; j++) c += "0123456789"[Math.floor(Math.random() * 10)];
      try { app.findFirstRecordByFilter("groups", "group_code = {:c}", { c: c }); }
      catch (e) { return c; }
    }
    throw new Error("code_gen_failed");
  },
  membership: function (app, gid, uid) {
    try { return app.findFirstRecordByFilter("group_members", "group = {:g} && user = {:u}", { g: gid, u: uid }); }
    catch (e) { return null; }
  },
  ...
  adminOf: function (app, gid) {
    try { return app.findFirstRecordByFilter("group_members", "group = {:g} && role = 'admin'", { g: gid }); }
    catch (e) { return null; }
  },
  notify: function (app, uid, type, text, meta) {
    const col = app.findCollectionByNameOrId("notifications");
    const r = new Record(col);
    r.set("user", uid); r.set("type", type); r.set("text", text);
    r.set("is_read", false); r.set("metadata", meta || {});
    app.save(r);
  },
  notifyGroup: function (app, gid, exceptUid, type, text, meta) {
    var members = app.findRecordsByFilter("group_members", "group = {:g}", "", 0, 0, { g: gid });
    for (var i = 0; i < members.length; i++) {
      var u = members[i].get("user");
      if (exceptUid && u === exceptUid) continue;
      try { this.notify(app, u, type, text, meta); } catch (e) {}
    }
  },
  fullName: function (app, uid) {
    try {
      const p = app.findFirstRecordByFilter("profiles", "user = {:u}", { u: uid });
      const n = ((p.get("first_name") || "") + " " + (p.get("last_name") || "")).trim();
      return n || "User";
    } catch (e) { return "User"; }
  },
  applyEffect: function (tx, v) {
    const gid = v.get("group"), type = v.get("type"), target = v.get("target_member");
    if (type === "admin-change") {
      ...
      h.set("group", gid); h.set("action", "admin_change"); h.set("voting", v.id); h.set("details", { new_admin: target });
      tx.save(h);
  ...
  isAppAdmin: function (auth) {
    ...
    var em = "";
    try { em = auth.email(); } catch (e) {}
    if (!em) { try { em = auth.get("email"); } catch (e) {} }
    return em === "koa2007@gmail.com";
  },
  adminUser: function (app) {
    try { return app.findFirstRecordByFilter("users", "email = {:e}", { e: "koa2007@gmail.com" }); } catch (e) { return null; }
  },
  cnt: function (app, col, filter, params) {
    try { return app.findRecordsByFilter(col, filter || "id != ''", "", 0, 0, params || {}).length; } catch (e) { return 0; }
  },
  ...
  completeFreeze: function (tx, fresh, gid) {
    const self = this; const vid = fresh.id;
    const finish = function (result, adminNote) {
      fresh.set("status", "completed"); fresh.set("result", result); fresh.set("completed_at", new Date().toISOString()); tx.save(fresh);
      if (adminNote) { const adm = self.adminOf(tx, gid); if (adm) self.notify(tx, adm.get("user"), "freeze_result", adminNote, { group_id: gid, voting_id: vid }); }
    };
    if (self.distinctObjections(tx, vid) >= 2) { finish("rejected", "Виключення скасовано: учасники заперечили"); return; }
    const targets = tx.findRecordsByFilter("freeze_targets", "voting = {:v}", "", 0, 0, { v: vid });
    const uids = targets.map(function (t) { return t.get("user"); });
    let willExclude = 0;
    for (const u of uids) { const m = self.membership(tx, gid, u); if (m && !m.get("is_observer") && !m.get("is_frozen")) willExclude++; }
    if (self.activeVoters(tx, gid) - willExclude < 2) { finish("rejected", "Виключення скасовано: залишилось би замало активних учасників"); return; }
    for (const u of uids) {
      const m = self.membership(tx, gid, u);
      if (m && !m.get("is_frozen")) { m.set("is_frozen", true); m.set("frozen_until", ""); tx.save(m); }
      const h = new Record(tx.findCollectionByNameOrId("group_history"));
      h.set("group", gid); h.set("action", "member_excluded"); h.set("voting", vid); h.set("details", { user: u });
      tx.save(h);
    }
    finish("accepted", null);
    self.notifyGroup(tx, gid, null, "member_excluded", "Учасника(ів) виключено з підрахунку за результатами голосування", { group_id: gid, voting_id: vid });
  },
    ...
    for (const v of due) {
      try {
        app.runInTransaction((tx) => {
          ...
          self.notifyGroup(tx, gid, null, "voting_completed",
            "Голосування завершено: " + (fresh.get("title") || "") + " \u2014 " + (accepted ? "прийнято" : "відхилено"),
            { group_id: gid, voting_id: v.id, result: accepted ? "accepted" : "rejected" });
        });
      } catch (er) {}
    }
  }
